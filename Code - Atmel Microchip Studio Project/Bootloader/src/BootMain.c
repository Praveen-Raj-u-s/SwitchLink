...
FRESULT res, resA, resB, boot_res;                                   // Holds the result of the FATFS functions done on the SD CARD TEST
...
static bool checkCRC(uint8_t rBuffer, int i){
	...
	*((volatile unsigned int*) 0x41007058) &= ~0x30000UL;
	...
	//CRC of SD Card
	enum status_code crcres = dsu_crc32_cal	(rBuffer	,256, &resultCrcSd); 
	...
	*((volatile unsigned int*) 0x41007058) |= 0x20000UL;
	...
	//CRC of memory (NVM)
	uint32_t resultCrcNvm = 0;
	crcres |= dsu_crc32_cal	(APP_START_ADDRESS + i	,256, &resultCrcNvm);
	
	return resultCrcNvm == resultCrcSd;
}
